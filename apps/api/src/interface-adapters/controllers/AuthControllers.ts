...
export class AuthController {
  ...
  console.log("========================================");
      ...
      return res.status(403).json({ error: error.message || "Falha na autenticação via Google." });
    }
  }
 ...
 async createByAdmin(req: Request, res: Response) {
    ...
    try {
      ...
      this.emailService.sendWelcomeEmail(email, tempPassword).then((success) => {
        if (success) {
          console.log(`[EMAIL WORKER] E-mail de boas-vindas enviado com sucesso para: ${email}`);
        } else {
          console.error(`[EMAIL WORKER] Erro crítico ao processar envio para: ${email}`);
        }
      });

      // 🔐 Defesa Rígida: 'tempPassword' REMOVIDA do retorno JSON para o administrador
      return res.status(201).json({ 
        message: "Usuário pré-autorizado criado com sucesso. O e-mail de acesso foi enviado ao destinatário.", 
        user: {
          id: user.id,
          email: user.email,
          role: user.role
