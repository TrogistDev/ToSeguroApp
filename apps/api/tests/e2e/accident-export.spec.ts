...
test.describe("API: Accident Export", () => {
  ...
  test("should require authentication to export accident data", async ({
    request,
  }) => {
    const response = await request.get(
      `${API_URL}/accidents/${ACCIDENT_ID}/export`,
    );
    expect(response.status()).toBe(401);
  });

  test("should return complete export DTO for authenticated tenant", async ({
    request,
  }) => {
    const response = await request.get(
      `${API_URL}/accidents/${ACCIDENT_ID}/export`,
