...
export const AdminDashboard: React.FC = () => {
  ...
  const [successMsg, setSuccessMsg] = useState<string | null>(null); // ✅ Adicionar este para dar feedback limpo
  const [errorMsg, setErrorMsg] = useState<string | null>(null);

  // Busca os sinistros estritamente isolados pelo Tenant ativo
  useEffect(() => {
      ...
      try {
        const res = await axios.get("http://localhost:3000/api/accidents", {
          headers: {
            ...
            "X-Tenant-Slug": tenantSlug, // Passando o contexto no cabeçalho customizado
          },
        });
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMsg(err.response?.data?.error || "Falha ao criar usuário.");
    }
  };
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center flex-wrap gap-2 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Gerenciamento de acessos e auditoria de sinistros de sua
            organização.
          </p>
        </div>
      ...
      {/* Tabela / Listagem de Sinistros Coletados */}
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
        <div className="p-5 border-b border-slate-100 bg-slate-50/50">
          <h3 className="font-bold text-slate-800">
            Histórico de Ocorrências Recebidas
          </h3>
        </div>
        {accidents.length === 0 ? (
          <div className="p-12 text-center text-slate-400 text-sm">
            Nenhum sinistro registrado neste tenant até o momento.
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-slate-100 text-sm">
                {accidents.map((acc: any) => (
                  <tr key={acc.id} className="hover:bg-slate-50/80 transition">
                    ...
                    <td>
                      <button onClick={() => handleExport(acc.id)}>
                        📥 Download JSON
                      </button>
                    </td>
                  </tr>
      ...
      {/* Modal de Cadastro Administrativo (Simplificado para manter clean) */}
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 animate-in fade-in duration-200">
          <div className="bg-white p-6 rounded-2xl shadow-2xl border border-slate-100 w-full max-w-md max-h-[90vh] overflow-y-auto animate-in zoom-in-95">
            ...
            {/* ✅ Banner de Sucesso Seguro e Elegante */}
            {successMsg && (
            ...
            <form onSubmit={handleCreateUser} className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full p-2 border border-slate-200 rounded-lg mt-1 bg-white outline-none"
                >
                  <option value="USER">
                    USER (Apenas envia relatórios do veículo)
                  </option>
                  <option value="ADMIN">
                    ADMIN (Gerencia acessos e visualiza tudo)
                  </option>
                </select>
              </div>
              <Button
