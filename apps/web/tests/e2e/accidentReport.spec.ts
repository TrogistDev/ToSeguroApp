...
test.describe("Fluxo do Formulário de Relato de Sinistro (AccidentReportForm)", () => {
  ...
  test("Deve renderizar o passo 1 e avançar para o passo 2", async ({
    page,
  }) => {
    ...
    await page.fill(
      'input[placeholder="Digite o endereço ou use GPS"]',
      "Av. Paulista, 1000",
    );
  ...
  test("Deve submeter relatório e exibir alerta de sucesso", async ({
    page,
  }) => {
    await page.fill('input[placeholder="Su nombre"]', "Gustavo");
    await page.fill('input[placeholder="Apellido"]', "Developer");
